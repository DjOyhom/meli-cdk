...
class Resources extends Construct {
    constructor(scope, id, props) {
        ...
        ///////Proxy lambda Path - /
        ///////Root Lambda Path - /
        const lambda_raiz = new apigateway.LambdaIntegration(props.lambda_proxy, {
            requestTemplates: { "application/json": '{ "statusCode": "200" }' }
        });
        api.root.addMethod('ANY', lambda_raiz); // para peticiones a la raíz ("/")
        
        ///////Proxy lambda for catch-all
        const lambda_proxy = new apigateway.LambdaIntegration(props.lambda_proxy, {
            requestTemplates: { "application/json": '{ "statusCode": "200" }' }
        });
        api.root.addProxy({
            defaultIntegration: lambda_proxy,
            anyMethod: true
        });
        ///////Proxy lambda--------------------------------------------------------------------------------------------------------
